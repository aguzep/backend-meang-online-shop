...
class UsersService extends ResolversOperationsService {
    ...
    async login() {
        try {
            ...
            const passwordCheck = bcrypt.compareSync(variables?.password as string, user.password); // CHECKEAR QUE ESTO "variables?.password as string" esté bien
    ...
    async register() {
        ...
        if (user?.password === null ||
            ...
            return {
                status: false,
                message: 'Usuario sin password coorecto, procura definirlo',
                user: null
            };
        }
        ...
        if (userCheck !== null) {
            return {
                status: false,
                message: `El email ${user?.email} está registrado y no puede registarte con este email`,
                user: null
            };
        }
    ...
    async delete() {
        ...
        if (id === undefined || id === '') {
            return {
                status: false,
                message: 'ID del usuario no definido, procura definirlo para eliminar el ususario',
                user: null
            };
        }
    ...
    async unblock(unblock: boolean, admin: boolean) {
        ...
        if (unblock && !admin) {
            console.log('Soy cliente y etoy cambiando la pass');
